...
import { CHAT_MESSAGES, PARENT } from "@/lib/caregivev3/mock-data";

export default function CaregiverV3ChatPage() {
  ...
  const send = () => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: `msg-${Date.now()}`, sender: "caregiver", text: input.trim(), time },
    ]);
    setInput("");
  };
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-cg-bg">
      {/* Chat header */}
      <div className="shrink-0 bg-white px-6 py-3 shadow-sm">
            ...
            >
              ...
              <p
                className={`text-[9px] mt-1 ${
                  msg.sender === "caregiver" ? "text-white/60 text-right" : "text-gray-400"
                }`}
              >
                {msg.time}
              </p>
      ...
      {/* Input */}
      <div className="shrink-0 bg-white px-4 py-2.5 border-t border-gray-100">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-full bg-[#f4f5f6] px-4 py-2.5 text-xs text-gray-700 placeholder:text-gray-400 outline-none"
          />
          <button
            onClick={send}
