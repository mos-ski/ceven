 ...
 <div className="relative flex flex-1 flex-col bg-cg-bg">
  ...
  {tab === "photos" ? (
   ...
   {photos.map((photo) => (
    ...
    className="relative overflow-hidden rounded-xl break-inside-avoid active:scale-[0.97] transition-transform aspect-square"
    >
    <SafeImage
     src={photo.image}
     alt={photo.caption}
   ...
   <div key={date} className="mb-5">
    ...
    {datePhotos.map((photo) => (
     ...
     key={photo.id}
     onClick={() => setActiveIndex(photos.findIndex((p) => p.id === photo.id))}
     className="overflow-hidden rounded-2xl bg-white text-left active:scale-[0.98]"
     >
