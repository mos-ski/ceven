...
function AlphanumericOtpInput({
 ...
 <div className="space-y-3">
  ...
  {Array.from({ length }).map((_, index) => (
   ...
   maxLength={1}
   spellCheck={false}
   className="h-12 min-w-0 flex-1 rounded-xl border border-gray-200 bg-white text-center text-base font-black uppercase text-cg-brand outline-none focus:border-cg-brand focus:ring-2 focus:ring-cg-brand/10"
   />
  ))}
  </div>
...
function ProblemState({ problem }: { problem: InviteProblem }) {
 ...
 <div className="flex flex-1 flex-col bg-cg-bg">
  ...
  <div className="w-full rounded-3xl bg-white p-6 text-center ">
