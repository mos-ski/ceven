...
function ReactionPicker({ onSelect, onClose }: { onSelect: (emoji: string) => void; onClose: () => void }) {
 ...
 <div className="flex items-center gap-1 rounded-full bg-white px-3 py-2 ring-1 ring-gray-100">
 {REACTIONS.map((r) => (
...
function PostCard({ item, onImageClick }: { item: FeedItem; onImageClick: (images: string[], idx: number) => void }) {
 ...
 <div className="mt-3 relative">
  ...
  <span className="text-6xl animate-bounce-in drop-">❤️</span>
  </div>
  )}
 </div>
 )}
...
export default function MomentsPage() {
 ...
 className="absolute bottom-24 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-cg-brand shadow-cg-brand/30 active:scale-95 transition-transform">
