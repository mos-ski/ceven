...
export default function ParentMedicationPage() {
 ...
 return (
  <div className="flex min-h-0 flex-1 flex-col bg-[#fffefa]">
   <div className="flex items-center justify-between bg-white px-5 pt-12 pb-4 ">
   ...
   <div className="flex-1 overflow-y-auto px-5 py-4">
    ...
    <div className="mb-4 flex gap-3">
     {[
      ...
      <div key={label} className="flex-1 rounded-2xl bg-white p-3 text-center ">
     ...
     {([["today", "Today"], ["history", "History"], ["requests", "Active Meds"]] as const).map(([key, label]) => (
      <button
       ...
       className={`flex flex-1 items-center justify-center rounded-md py-2 text-xs font-semibold transition-colors ${
        activeTab === key ? "bg-white text-cg-brand " : "text-gray-500"
       }`}
      >
       {label}
      </button>
    ...
    {activeTab === "requests" && (
     <div className="flex flex-col gap-2">
      {requests.length === 0 ? (
       ...
       requests.map((req) => (
        <div key={req.id} className="rounded-2xl bg-white p-4 ">
