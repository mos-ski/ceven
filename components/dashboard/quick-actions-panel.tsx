...
export default function QuickActionsPanel({ actions, onAction, onCustomize }: Props) {
 ...
 <div className="flex flex-col gap-4 rounded-xl border border-[#e0bfa0] bg-[#faf2e1] px-4 py-4 shadow-[0_12px_40px_-12px_rgba(12,12,13,0.4)]">
  ...
  <h3 className="font-[family-name:var(--font-urbanist)] text-base font-medium text-black">Quick Actions</h3>
  <button
   onClick={onCustomize}
  ...
  {actions.map((key) => {
   ...
   >
    ...
    <span className="text-center font-[family-name:var(--font-urbanist)] text-xs font-bold text-[#3b2513]">
    {def.label}
    </span>
   </button>
   );
