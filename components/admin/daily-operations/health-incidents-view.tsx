    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="flex flex-col gap-4 overflow-y-auto px-6 py-5">
          <div className="flex flex-col gap-1.5">
            ...
            >
              ...
              {INCIDENTS.map((inc) => (
                <option key={inc.id} value={inc.child}>{inc.child}</option>
              ))}
            </select>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => onOpenChange(false)} className="h-9 rounded-lg bg-[#3b2513] px-4 font-[family-name:var(--font-urbanist)] text-sm font-medium text-[#faf2e1]">Submit Report</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
          ...
          <>
            <div className="flex items-center justify-between gap-4 bg-[#faf2e1] px-6 py-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className={STATUS_BADGE_CLASS[incident.status]}>● {incident.status}</Badge>
              </div>
            </div>
...
function IncidentRow({ incident, onView }: { incident: Incident; onView: (incident: Incident) => void }) {
  return (
    <TableRow onClick={() => onView(incident)} className="cursor-pointer">
      <TableCell onClick={(e) => e.stopPropagation()}>
        <input type="checkbox" className="h-4 w-4 accent-[#3b2513]" />
      </TableCell>
      <TableCell>
        <p className="font-[family-name:var(--font-urbanist)] text-sm font-semibold text-black">{incident.child}</p>
        <p className="font-[family-name:var(--font-urbanist)] text-[10px] text-[#9ca3af]">{incident.childInfo}</p>
      </TableCell>
      <TableCell className="font-[family-name:var(--font-urbanist)] text-sm text-[#6b7280]">{incident.type}</TableCell>
      <TableCell>
      ...
      <TableCell className="font-[family-name:var(--font-urbanist)] text-sm text-[#6b7280]">{incident.time}</TableCell>
      <TableCell className="font-[family-name:var(--font-urbanist)] text-sm text-[#6b7280]">{incident.reportedBy}</TableCell>
      <TableCell className="font-[family-name:var(--font-urbanist)] text-sm text-[#6b7280]">{incident.parentNotified ? "Yes" : "No"}</TableCell>
      <TableCell><Badge variant="outline" className={STATUS_BADGE_CLASS[incident.status]}>● {incident.status}</Badge></TableCell>
      <TableCell>
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card padding="none">
        <div className="flex flex-wrap items-center justify-between gap-4 p-4">
          <h2 className="font-[family-name:var(--font-merriweather)] text-lg font-bold text-[#2d1810]">Incident Log</h2>
          <div className="flex items-center gap-2">
        ...
        <div className="hidden overflow-x-auto lg:block">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-10"><input type="checkbox" className="h-4 w-4 accent-[#3b2513]" /></TableHead>
            ...
            <TableBody>
              {filteredIncidents.length === 0 ? (
                <TableRow><TableCell colSpan={9} className="py-10 text-center font-[family-name:var(--font-urbanist)] text-sm text-[#9ca3af]">No incidents match your search or filters.</TableCell></TableRow>
              ) : (
                filteredIncidents.map((incident) => (<IncidentRow key={incident.id} incident={incident} onView={setViewingIncident} />))
              )}
            </TableBody>
          </Table>
        </div>
        ...
        <div className="flex flex-col gap-2 px-4 pb-4 lg:hidden">
          {filteredIncidents.length === 0 && (
            <p className="py-6 text-center font-[family-name:var(--font-urbanist)] text-sm text-[#9ca3af]">No incidents match your search or filters.</p>
          )}
          {filteredIncidents.map((incident) => (
            <div key={incident.id} onClick={() => setViewingIncident(incident)} className="cursor-pointer rounded-xl bg-[#F5EDD8]/30 p-3">
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-[family-name:var(--font-urbanist)] text-sm font-semibold text-[#2d1810]">{incident.child}</span>
                  <p className="font-[family-name:var(--font-urbanist)] text-[10px] text-[#9ca3af]">{incident.childInfo}</p>
                </div>
                <Badge variant="outline" className={STATUS_BADGE_CLASS[incident.status]}>● {incident.status}</Badge>
              </div>
              ...
              <p className="mt-1 font-[family-name:var(--font-urbanist)] text-[10px] text-[#9ca3af]">{incident.time} • by {incident.reportedBy}</p>
            </div>
          ))}
        </div>
      </Card>
