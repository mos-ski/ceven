  ...
  renderVals() {
    ...
    return {
      ...
      audit: s.audit.map(a => ({...a,
        dot: {width: 10, height: 10, flex: 'none', borderRadius: '50%', marginTop: 4, background: (a.cat === 'AI' || a.cat === 'LEARN') ? '#9A6033' : a.cat === 'MEDIA' ? '#3B82F6' : a.cat === 'MONEY' ? '#FF9A01' : a.cat === 'DUTY' ? '#3B2513' : '#009061'},
        badge: {fontFamily: 'Urbanist', fontSize: 10, fontWeight: 700, letterSpacing: '.06em', padding: '3px 9px', borderRadius: 99, background: (a.cat === 'AI' || a.cat === 'LEARN') ? '#F7E8D4' : a.cat === 'MEDIA' ? '#EFF6FF' : a.cat === 'MONEY' ? '#F9F1E6' : a.cat === 'DUTY' ? '#F1E6D6' : '#E1F5EC', color: (a.cat === 'AI' || a.cat === 'LEARN') ? '#9A6033' : a.cat === 'MEDIA' ? '#3B82F6' : a.cat === 'MONEY' ? '#FF9A01' : a.cat === 'DUTY' ? '#3B2513' : '#009061'},
      })),
